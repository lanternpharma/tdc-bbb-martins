...
# function to train logistic regression model for predictions

def train_logistic(train_df, val_df, scale_method):
...
full_train_data = pd.concat([
                             ...
                             fingerprint_maccs_full_train.drop(['Drug_ID', 'target'], axis=1)],  axis=1)
...
full_test_data = pd.concat([
                             ...
                             fingerprint_maccs_test.drop(['Drug_ID', 'target'], axis=1)], axis=1)
...
train_only_for_fs = pd.concat([
                             ...
                             fingerprint_maccs_train_only.drop(['Drug_ID', 'target'], axis=1)], axis=1)
...
for seed in [1, 2, 3, 4, 5]:
    ...
    trained_model = train_logistic(full_train_data_ml_augmented[['target'] + logistic_features_selected], full_valid_data_ml[['target'] + logistic_features_selected], "None")
    
    #Make predictions on the test set
    predictions = {}
    name = benchmark['name']
    predicted_prob, predicted_class = logistic_predict(trained_model, full_test_data_ml[['target'] + logistic_features_selected], "None")
