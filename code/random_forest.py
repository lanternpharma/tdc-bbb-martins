...
trainval_predicted_prob = cross_val_predict(best_model, X=full_train_data_ml_scaled[lasso_features_selected], y=full_train_data_ml_scaled['target'], cv=10, method='predict_proba')[:,1]
trainval_predicted_class = [1 if pred > 0.5 else 0 for pred in trainval_predicted_prob]

trainval_predictions_df = pd.DataFrame({'Drug_ID':full_train_data_ml_scaled.index.values, 'Actual_value':full_train_data_ml_scaled.target, 'Predicted_prob':trainval_predicted_prob, 'Predicted_class':trainval_predicted_class})
