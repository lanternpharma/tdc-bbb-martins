...
def train_rf(train_df, val_df, scale_method):
    import optuna.integration.lightgbm as lgb
    ...
    rkf = RepeatedKFold(n_splits=10, n_repeats=fold_repeats, random_state=global_seed)
    ...
    def objective(trial):
        ...
        cv_score = np.mean(cross_val_score(rfcv, X, y, n_jobs=-1, cv=rkf, scoring="roc_auc"))
    ...
    search_model_retrained = final_model
    search_model_retrained.fit(X_train, y_train)
...
for seed in [1, 2, 3, 4, 5]:
    ...
    trained_model = train_rf(full_train_data_ml_augmented[['target'] + logistic_features_selected], full_valid_data_ml[['target'] + logistic_features_selected], "None")
    
    #Make predictions on the test set
    predictions = {}
    name = benchmark['name']
    predicted_prob, predicted_class = rf_predict(trained_model, full_test_data_ml[['target'] + logistic_features_selected], "None")
    ...
    val_predicted_prob, val_predicted_class = rf_predict(trained_model, full_valid_data_ml[['target'] + logistic_features_selected], "None")
