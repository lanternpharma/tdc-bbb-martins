...
#The determinism ection below ensures that the nn results will be repeatable given the same inputs
...
print("Lasso features selected on augmented trainval: ",len(lasso_features_selected))
...
def svc_hyper_search(train_df, scale_method):
    ...
    def objective(trial):
        ...
        params = {
            ...
            'C': trial.suggest_float("C", 2, 5),
            'gamma': trial.suggest_float("gamma", 0.001, 0.01),

        }
    
        # Run model for the hyperparameter values
        model = SVC(random_state=global_seed,  **params)
    
        cv_score = np.mean(cross_val_score(model, X, y, n_jobs=-1, cv=sss, scoring="neg_log_loss"))

        #if cv_score > best_score:
        #    training_rounds = len( list(model.values())[0] )
    ...
    study.optimize(objective, n_trials=100) # limit number of searches because running all trials takes days and doesn't improve scores
    ...
    final_model = SVC(random_state=global_seed, **best_params)

    return final_model, study
        
# Function to make predictions 

def svc_fit(model, df, scale_method):
...
def svc_predict(trained_model, df, scale_method):
...
#best_model, study = svc_hyper_search(full_train_data_ml_kpca[['target'] + lasso_features_selected_aug], "None")
#disable hyperparameter search to reduce run time, use best params from results of this search in previous run

best_params = {'kernel': 'linear', 'probability': True, 'degree': 4, 'C': 2.000532802697363, 'gamma': 0.0058080700970346184}
best_model = SVC(random_state=global_seed, **best_params)
...
for seed in [1, 2, 3, 4, 5]:
    ...
    best_model_fit = svc_fit(best_model, train_df[['target'] + lasso_features_selected], "None")
    
    #Make predictions on the test set
    predictions = {}
    name = benchmark['name']
    predicted_prob, predicted_class = svc_predict(best_model_fit, test_df[['target'] + lasso_features_selected], "None")
...
#trainval_predicted_prob = cross_val_predict(best_model, X=X_trainval, y=y_trainval, cv=2, method='predict_proba')[:,1]
